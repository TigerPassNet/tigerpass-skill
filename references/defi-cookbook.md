...
# 6. Authorize builder fee
tigerpass hl approve-builder

# === Ready to trade ===

# 7. Check balance on L1 (NOT HyperEVM balance!)
tigerpass hl info --type balances
...
|-----------|-----|--------------|
| Swap tokens on any DEX | `tigerpass swap` (automatic routing, best price) | EVM / safeAddress |
...
| Bridge USDC cross-chain | `tigerpass bridge --from X --to Y` (Circle CCTP V2, 5 chains) | EVM / safeAddress |
...
| Transfer EVM tokens | `tigerpass pay --to 0x... --amount X` | EVM / safeAddress |
| Transfer on HyperEVM | `tigerpass pay --chain HYPEREVM` (EOA direct) | HyperEVM / eoaAddress |
| Transfer SUI/tokens on Sui | `tigerpass pay --chain SUI --amount X` | Sui / suiAddress |
| Stake SUI to validator | `tigerpass sui stake --amount X --validator 0x...` | Sui / suiAddress |
| Unstake SUI | `tigerpass sui unstake --staked-object 0x...` | Sui / suiAddress |
| View Sui stakes/validators | `tigerpass sui info --type stakes\|validators` | Sui / suiAddress |
| Transfer Sui object | `tigerpass sui transfer --object 0x... --to 0x...` | Sui / suiAddress |
| Execute on HyperEVM | `tigerpass exec --chain HYPEREVM` (EOA direct) | HyperEVM / eoaAddress |
| Supply/borrow on lending protocols | `tigerpass exec` with AAVE/Compound ABI | EVM / safeAddress |
| Interact with any EVM contract | `tigerpass exec --fn` or `--data` | EVM / safeAddress |
| Read EVM on-chain state (no gas) | `tigerpass call --fn` | EVM |
| Sign message for Sui | `tigerpass sign-message --message "..." --chain SUI` | Sui / suiAddress |
