...
- **Not on HyperEVM** — swap is not available on HyperEVM (no 0x support)
...
HyperEVM is Hyperliquid's native EVM chain (chain ID 999 mainnet, 998 testnet). All chains now default to EOA transactions. HyperEVM remains **EOA-only** (no Safe support).

Chain type detection is automatic: all commands use EOA by default, `--safe` opts into Safe on supported chains.

### Key Differences: EOA Default vs HyperEVM

| | EOA on Safe-supported chains (Base, Ethereum, etc.) | HyperEVM |
|---|---|---|
| **Transaction source** | EOA (`eoaAddress`, default) | EOA (`eoaAddress`) |
| **Safe opt-in** | `--safe` flag available | Not available (EOA-only) |
| **Gas payment** | EOA pays in native token | EOA pays in HYPE |
| **Signing** | secp256k1 (default), Safe UserOp (`--safe`) | EIP-155 legacy tx |
| **Batch exec** | Atomic via Safe (`--safe`), sequential otherwise | Sequential (non-atomic) |
...
5. **Verify allowance** — `tigerpass approve --token X --spender 0x...` (without `--amount`) after approve, before the main transaction
...
|-----------|-----|--------------|
| Swap tokens on any DEX | `tigerpass swap` (automatic routing, best price) | EVM / eoaAddress (default) |
...
| Bridge USDC cross-chain | `tigerpass bridge --from X --to Y` (Circle CCTP V2, 5 chains) | EVM / eoaAddress (default) |
...
| Transfer EVM tokens | `tigerpass pay --to 0x... --amount X --token <TOKEN>` | EVM / eoaAddress (default) |
...
| Supply/borrow on lending protocols | `tigerpass exec` with AAVE/Compound ABI | EVM / eoaAddress (default) |
| Interact with any EVM contract | `tigerpass exec --fn` or `--data` | EVM / eoaAddress (default) |
